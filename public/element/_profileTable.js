window.profileTable = {
    make : function (icon, title, sub, imgSource=false) {
        ...
        if (typeof title === 'string') text_obj.appendChild(title);
        else text_obj.innerHTML = title;
        if (typeof sub === 'string') subs_obj.appendChild(sub);
        else subs_obj.innerHTML = sub;
